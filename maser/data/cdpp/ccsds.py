...
def decode_ccsds_date(p_field, t_field, epoch=None) -> 'CCSDSDate':
    ...
    if time_code_id == 1:
        ...
        raise NotImplementedError('CCSDS Agency Defined level 2')  # This is CCSDS AgencyDefined format
    else:
...
class CCSDSDate(object):
    ...
    * In CCS format, the values in the `t_field` are encoded in _Binary Coded Decimal (BCD)_. This means that the
    ...
    def __init__(self, p_field, t_field):
        ...
        time_code_level_values = ["_UNK_", "CUC", "CUC", "_UNK_", "CDS", "CCS", "AGENCY_DEFINED", "_UNK_"]
...
class CCSDSDateCUC(CCSDSDate):
    ...
    def __init__(self, p_field, t_field):
        ...
        if self._n_bytes_t_field != self._n_bytes_coarse_time + self._n_bytes_fine_time:
            raise Exception('T_field length does match P_field time format specification')
        self.time_scale = "TAI"
    ...
    def _decode_t_field(self) -> datetime:
        ...
        microseconds = int((sub_seconds_fraction_counter*1e6) / sub_seconds_fraction_total)

        if self._n_bytes_fine_time == 3:
            print("{}: {}".format("Warning", "Python datetime module doesn't handle sub-microsecond accuracy."))

        return self.time_epoch + datetime.timedelta(seconds=seconds) + datetime.timedelta(microseconds=microseconds)
...
class CCSDSDateCDS(CCSDSDate):
    ...
    def __init__(self, p_field, t_field, epoch=None):
        ...
        if self._n_bytes_t_field != self._n_bytes_day + self._n_bytes_millisecond + self._n_bytes_sub_millisecond:
            raise Exception('T_field length does match P_field time format specification')
        self.time_scale = "UTC"
    ...
    def _decode_t_field(self) -> datetime:
        ...
        return self.time_epoch + datetime.timedelta(days=days) \
            + datetime.timedelta(seconds=seconds) + datetime.timedelta(microseconds=microsec)
...
class CCSDSDateCCS(CCSDSDate):
    ...
    def _decode_t_field(self):
        ...
        if self._calendar_variation_flag:
            ...
            dt = datetime.datetime(year, 1, 1, hour, minute, second) + datetime.timedelta(days=day)
        else:
